...
ソースコード(html,md,scss,js)は見やすいように整理したり、できることを増やしたり、
大改造したので原型を留めていません。

なお、長すぎるので読みたくない方は
[次の見出し](#lighthousewebサイト評価ツールによる評価)
へジャンプできます。

余談）
テーブルにスクロールバーを表示させて見出しを固定させたかったんですが、
前者は現状のmd記法では無理で独自拡張されたmd記法じゃないと記述できなさそう。
もしくはmdに直接htmlを記述しないとダメ？邪道な気がして手が出せません。
...
| 06  | 画面共通 - ヘッダ     | サイト名                     | 〇     | 〇     | サイト名を表示する。                                                                                                     | 
| 07  |                       | ナビバー                     | 〇     | 〇     | ナビバーを表示する。                                                                                                     | 
| 08  | 画面共通 - トップ     | トップ画像                   | ×     | 〇     | トップ画像を表示する。                                                                                                   | 
| 09  | 画面共通 - サイドバー | 記事検索                     | ×     | 〇     | サイト内で記事を検索するフォームを表示する。                                                                             | 
...
所感では大分良くなったと思っていましたが、点数からも良くなったことが分かったので嬉しいです。

後は評価項目以外でも評価できる箇所(各ページの情報の出し方や検索機能など)はあるので、
使ってみてどうなるかというところですね。
